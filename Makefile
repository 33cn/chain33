...
PKG_LIST_GOSEC := `go list ./... | grep -v "vendor" | grep -v "mocks" | grep -v "cmd" | grep -v "types" | grep -v "commands" | grep -v "bipwallet" | grep -v "log15" | grep -v "ed25519" | grep -v "crypto"`
...
gosec:
	@gosec -nosec=true ${PKG_LIST_GOSEC}
