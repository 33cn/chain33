...
func (bc *BaseClient) CheckBlock(block *types.BlockDetail) error {
	...
	//check block size and tx count and blockfee
	...
	if cfg.IsFork(block.Block.Height, "ForkMaxBlockFee") {
		if block.Block.Fee() > types.MaxBlockFee {
			return types.ErrBlockFee
		}
...
func (bc *BaseClient) AddTxsToBlock(block *types.Block, txs []*types.Transaction) []*types.Transaction {
	...
	maxFee := types.MaxBlockFee - 100000 //留下挖矿交易的手续费 1e5
	currentCount := int64(len(block.Txs))
	types.AssertConfig(bc.client)
	cfg := bc.client.GetConfig()
	maxTx := cfg.GetP(block.Height).MaxTxNumber
	addedTx := make([]*types.Transaction, 0, len(txs))
	var toalBlockFee int64 = 0
	for i := 0; i < len(txs); i++ {
		if cfg.IsFork(block.Height, "ForkMaxBlockFee") {
			toalBlockFee += txs[i].GetTxFee()
			if toalBlockFee > maxFee {
				return addedTx
			}
		}
