...
func TestNone_CheckTx(t *testing.T) {
	...
	commit := &nty.CommitDelayTx{RelativeDelayTime: -1, DelayTx: common.ToHex(types.Encode(delayTx))}
...
func TestNone_Exec_CommitDelayTx(t *testing.T) {
	...
	addr, priv := util.Genaddress()
	delayTx := util.CreateNoneTx(cfg, priv)
	commit := &nty.CommitDelayTx{DelayTx: common.ToHex(types.Encode(delayTx)), RelativeDelayTime: 10, IsBlockHeightDelayTime: true}
	...
	require.True(t, 1 == info.DelayBeginTime)
}
...
func TestNone_Query_GetDelayBeginTime(t *testing.T) {
	...
	msg, err := n.Query(nty.QueryGetDelayBeginTime, types.Encode(&types.ReqBytes{Data: delayTx.Hash()}))
	require.Nil(t, err)
