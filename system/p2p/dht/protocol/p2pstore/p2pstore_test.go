...
func TestInit(t *testing.T) {
	...
	for i := 0; i < 14; i++ {
		...
		msg := testGetBody(t, client, "p2p", info)
		require.Equal(t, 1000, len(msg.Data.(*types.BlockBodys).Items))

		msg2 := testGetBody(t, client, "p2p2", info)
...
func initEnv(t *testing.T, q queue.Queue) *Protocol {
	...
	m1, err := multiaddr.NewMultiaddr(fmt.Sprintf("/ip4/127.0.0.1/tcp/%d", port1))
	if err != nil {
		t.Fatal(err)
	}
	...
	m2, err := multiaddr.NewMultiaddr(fmt.Sprintf("/ip4/127.0.0.1/tcp/%d", port2))
	if err != nil {
		t.Fatal(err)
	}
	...
	exRT, _ := kbt.NewRoutingTable(20, kbt.ConvertPeerID(env2.Host.ID()), time.Minute, env2.Host.Peerstore(), 0, nil)
	p2 := &Protocol{
		P2PEnv:               &env2,
	...
	protocol.RegisterStreamHandler(p2.Host, requestPeerAddr, p2.handleStreamRequestPeerAddr)
	protocol.RegisterStreamHandler(p2.Host, responsePeerAddr, p2.handleStreamResponsePeerAddr)
	protocol.RegisterStreamHandler(p2.Host, fetchPeerAddr, protocol.HandlerWithRW(p2.handleStreamPeerAddr))
	protocol.RegisterStreamHandler(p2.Host, fetchActivePeer, protocol.HandlerWithWrite(p2.handleStreamFetchActivePeer))
