...
func loadLib(lib, pin, label string) (*pkcs11.Ctx, uint, *pkcs11.SessionHandle, error) {
	var slot uint = 0
	logger.Debug("Loading pkcs11 library [%s]\n", lib)
	if lib == "" {
	...
	for _, s := range slots {
		...
		logger.Debug("Looking for %s, found label %s\n", label, info.Label)
		if label == info.Label {
			found = true
			slot = s
			break
	...
	for i := 0; i < 10; i++ {
		...
		if err != nil {
			logger.Warn("OpenSession failed, retrying [%s]\n", err)
		} else {
			break
		}
	}
	if err != nil {
		logger.Crit("OpenSession [%s]\n", err)
	}
	logger.Debug("Created new pkcs11 session %+v on slot %d\n", session, slot)
...
func (csp *impl) getSession() (session pkcs11.SessionHandle) {
	...
	case session = <-csp.sessions:
		logger.Debug("Reusing existing pkcs11 session %+v on slot %d\n", session, csp.slot)
...
func (csp *impl) getECKey(ski []byte) (pubKey *ecdsa.PublicKey, isPriv bool, err error) {
	...
	if err != nil {
		isPriv = false
		logger.Debug("Private key not found [%s] for SKI [%s], looking for Public key", err, hex.EncodeToString(ski))
	}
...
func (csp *impl) generateECKey(curve asn1.ObjectIdentifier, ephemeral bool) (ski []byte, pubKey *ecdsa.PublicKey, err error) {
	...
	logger.Info("Generated new P11 key, SKI %x\n", ski)
...
func (csp *impl) importECKey(curve asn1.ObjectIdentifier, privKey, ecPt []byte, ephemeral bool, keyType bool) (ski []byte, err error) {
	...
	if keyType == publicKeyFlag {
		...
		logger.Debug("Importing Private EC Key [%d]\n%s\n", len(privKey)*8, hex.Dump(privKey))
...
func (csp *impl) getSecretValue(ski []byte) []byte {
	...
	if err != nil {
		logger.Warn("P11: get(attrlist) [%s]\n", err)
	}
	...
	logger.Warn("No Key Value found!", err)
	return nil
}
