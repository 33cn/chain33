...
// Use of c source code is governed by a BSD-style
...
func (c *CreateDappSourceTask) init() error {
	...
	if err != nil {
		return errors.New(fmt.Sprintf("Read file %s failed. error %q", c.ExecHeaderTempFile, err))
	}
...
func (c *CreateDappSourceTask) readActionMemberNames() error {
	...
	if index < 0 {
		return errors.New(fmt.Sprintf("Action %s Not Existed", c.ActionName))
	}
	...
	if len(c.actionInfos) == 0 {
		return errors.New(fmt.Sprintf("Can Not Find %s Member Info", c.ActionName))
	}
	return nil
}
