...
func testProcCreateNewAccount(t *testing.T, wallet *Wallet) {
	...
	cr, err := crypto.New(types.GetSignName("", wallet.SignType), wallet.GetLastHeader().GetHeight())
	require.NoError(t, err)
...
func testProcImportPrivKey(t *testing.T, wallet *Wallet) {
	...
	cr, err := crypto.New(types.GetSignName("", wallet.SignType), wallet.lastHeader.GetHeight())
	require.NoError(t, err)
