...
func maxmsgsize(cmd string) uint32 {
	...
	case "block":
		return 5e6 + 4 // max segwit block size 5MB
	case "getblocks":
	...
	case "getheaders":
		return 4 + 3 + 500*32 + 32 // we allow up to 500 locator hashes
	case "cmpctblock":
		return 0x10000 // 64KB shall be enough
	case "getblocktxn":
		return 0x10000 // 64KB shall be enough
	case "blocktxn":
		return 32 + 2 + 1e6 - 80 - 5 // all txs that can fit withing 1MB block
	default:
		return 1024 // Any other type of block: 1KB payload limit
	}
}
