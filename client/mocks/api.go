...
// AddSubscribeTxReceipt provides a mock function with given fields: param
func (_m *QueueProtocolAPI) AddSubscribeTxReceipt(param *types.SubscribeTxReceipt) (*types.ReplySubTxReceipt, error) {
	ret := _m.Called(param)

	var r0 *types.ReplySubTxReceipt
	if rf, ok := ret.Get(0).(func(*types.SubscribeTxReceipt) *types.ReplySubTxReceipt); ok {
		r0 = rf(param)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*types.ReplySubTxReceipt)
		}
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(*types.SubscribeTxReceipt) error); ok {
		r1 = rf(param)
	} else {
		r1 = ret.Error(1)
	}
