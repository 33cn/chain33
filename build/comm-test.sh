...
function createPrivacyPub2PrivTx() {
    ...
    amount=$3
    expire=$4
    result=$($name bty pub2priv -p "${keypair}" -a "${amount}" --expire "${expire}")
    returnStr1=$result
}
...
function createPrivacyPriv2PrivTx() {
    ...
    sender=$4
    expire=$5
    result=$($name bty priv2priv -p "${keypair}" -a "${amount}" -s "${sender}" --expire "${expire}")
    returnStr1=$result
}
...
function createPrivacyPriv2PubTx() {
    ...
    amount=$4
    expire=$5
    result=$($name bty priv2pub -f "${from}" -o "${to}" -a "${amount}" --expire "${expire}")
    returnStr1=$result
}
