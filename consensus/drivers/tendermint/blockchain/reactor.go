...
func NewBlockchainReactor(state sm.State, blockExec *sm.BlockExecutor, store *core.BlockStore, fastSync bool) *BlockchainReactor {
	if state.LastBlockHeight != store.Height() {
		panic(fmt.Sprint("Panicked on a Sanity Check: %v",fmt.Sprint("state (%v) and store (%v) height mismatch", state.LastBlockHeight, store.Height())))
	}
...
FOR_LOOP:
	for {
		...
		case request := <-bcR.requestsCh: // chan BlockRequest
			bcR.Logger.Info("BlockRequest coming", "request", request)
		...
		case peerID := <-bcR.timeoutsCh: // chan string
			bcR.Logger.Info("peer timeout coming", "peerID", peerID)
			// Peer timed out.
