...
func (pool *BlockPool) PopRequest() {
	...
	if r := pool.requesters[pool.height]; r != nil {
		...
		*/
		...
		panic(fmt.Sprint("Panicked on a Sanity Check: %v",fmt.Sprintf("Expected requester to pop, got nothing at height %v", pool.height)))
	}
}
...
func (pool *BlockPool) RedoRequest(height int64) {
	...
	if request.block == nil {
		panic(fmt.Sprint("Panicked on a Sanity Check: %v","Expected block to be non-nil"))
	}
...
func (pool *BlockPool) makeNextRequester() {
	...
	//pool.Logger.Info("makeNextRequester","height", nextHeight, "pool.height", pool.height)
