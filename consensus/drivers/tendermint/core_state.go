...
func NewConsensusState(client *drivers.BaseClient, blockStore *ttypes.BlockStore, state State, blockExec *BlockExecutor, evpool ttypes.EvidencePool) *ConsensusState {
	cs := &ConsensusState{
		client:           client,
		blockExec:        blockExec,
...
func (cs *ConsensusState) SetBroadcastChannel(broadcastChannel chan<- ttypes.ReactorMsg) {
...
func (cs *ConsensusState) handleMsg(mi MsgInfo) {
	...
	default:
		tendermintlog.Error("Unknown msg type", msg.TypeName(), "peerid", peerID, "peerip", peerIP)
	}
	if err != nil {
...
func (cs *ConsensusState) proposalHeartbeat(height int64, round int) {
	...
	for {
		...
		cs.broadcastChannel <- &ttypes.ProposalHeartbeatMessage{heartbeat}
		counter += 1
...
func (cs *ConsensusState) finalizeCommit(height int64) {
	...
	tendermintlog.Error("finalizeCommit block", "height", block.Height)
...
func (cs *ConsensusState) defaultSetProposal(proposalTrans *ttypes.ProposalTrans) error {
	...
	if (cs.Proposal != nil && cs.Proposal.Height == proposal.Height && cs.Proposal.Round < proposal.Round) ||
		(cs.Proposal == nil && proposal.Height == cs.Height && proposal.Round > cs.Round) {
		tendermintlog.Error("defaultSetProposal: a new round begin, change round", "height", proposal.Height, "other-round", proposal.Round)
...
func (cs *ConsensusState) addVote(vote *ttypes.Vote, peerID string) (added bool, err error) {
	...
	if vote.Height+1 == cs.Height {
		...
		if added {
			cs.broadcastChannel <- &ttypes.VoteMessage{vote}
...
func (cs *ConsensusState) GetPrevotesState(height int64, round int, blockID ttypes.BlockID) *ttypes.BitArray {
	if height != cs.Height {
		return nil
	}
...
func (cs *ConsensusState) GetPrecommitsState(height int64, round int, blockID ttypes.BlockID) *ttypes.BitArray {
	if height != cs.Height {
		return nil
	}
...
func (cs *ConsensusState) SetPeerMaj23(height int64, round int, type_ byte, peerID ID, blockID ttypes.BlockID) {
	if height == cs.Height {
