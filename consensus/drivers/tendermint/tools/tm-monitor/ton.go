...
func (o *Ton) printTable() {
	...
	fmt.Fprintln(w, "NAME\tHEIGHT\tONLINE\tVALIDATOR\tMEMINFO\tBLOCKTIME\tTXCOUNT\tSTATEHASH\t")
	for _, n := range o.monitor.Nodes {
		fmt.Fprintln(w, fmt.Sprintf("%s\t%d\t%v\t%v\t%d/%d(mb)\t%d\t%d\t%s", n.Name, n.Height, n.Online,
			false, n.MemInfo.Used, n.MemInfo.All, n.BlockTime, n.TxCount, n.StateHash))
	}
	w.Flush()
}
