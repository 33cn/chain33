...
func TestDuplicateMempool(t *testing.T) {
	...
	reply, err := mem.qclient.Wait(msg)
	t.Log("size:", len(reply.GetData().(*types.ReplyTxList).GetTxs()))
	for i, tx := range reply.GetData().(*types.ReplyTxList).GetTxs() {
		t.Log("tx:", i, tx)
	}
